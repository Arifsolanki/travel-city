...
const createTravelExpense = async (req, res) => {
  /// To create travel-expense
  try {
    const travelExpense = await TravelExpense.create({
      ...req.body,
      user: req.user._id,
    });
...
const updateTravelExpense = async (req, res) => {
  try {
    const travelExpense = await TravelExpense.findOneAndUpdate(
...
const deleteTravelExpenses = async (req, res) => {
  try {
    const travelExpense = await TravelExpense.findOneAndDelete({
      _id: req.params.id,
      user: req.user._id,
    });
